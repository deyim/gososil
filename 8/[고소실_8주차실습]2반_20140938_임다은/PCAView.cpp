...
void CPCAView::OnProblem1()
{
	...
	//공식 (2) 또는 (3)을 이용하여 입력된 데이터의 Covariance matrix를 구한다. n개의 샘플의 평균값을 빼주는 것에 주의한다.
	//dataMean.at<float>(i,0)/200
	...
	for (i=0; i<10; i++) {
		...
		dataMean.at<float>(i,0) = t1/(float)200;
	}
	...
	//OpenCV에서 제공하는 함수를 사용하여 Covariance matrix로부터 eigenvalue와 eignevector를 구한다. 
	//구해진 eigenvalue를 내림차순으로 정렬하고, eigenvector를 대응하는 eigenvalue의 순으로 정렬한다. 
	// ADD YOUR CODE HERE
	for(i = 0 ; i < 10; i++){
		for(j = 0 ; j < 200 ; j++){
	...
	//eigen decomposition
	/// e: eigen value, V : eigen vector for each
