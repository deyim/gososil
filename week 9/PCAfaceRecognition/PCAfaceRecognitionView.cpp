...
void CPCAfaceRecognitionView::OnFacerecognitionTrain()
{
	...
	inputDir = "../DBlab12/train1";  // input directory
...
void CPCAfaceRecognitionView::OnFacerecognitionTest()
{
	...
	*****/
	...
	for (i=0; i<Proj_q.cols; i++) {
		Mat t1(Proj_q, Rect(i, 0, 1, Proj_q.rows));
		for (j=0; j<Proj_p.cols; j++) {
			Mat t2(Proj_p, Rect(j, 0, 1, Proj_p.rows));
			Mat t3 = t1 - t2;
			t3 = t3.mul(t3); // element wise multiplication (square)
...
void CPCAfaceRecognitionView::OnTest2()
{
	...
	*****/
	...
	// TODO 1: Complete this part for the final test
	// read training data and images in DBlab12/test1 and perform face recognition
	/*	여기 해야돼3	*/
	// OnTest 로 동일하게 하되, 불러올 데이터 경로만 DBlab12/validate1 --> DBlab12/test1로
	display = 0;
}
