...
void transform_points_AOS( INOUT POINT_ELEMENT *p_AOS, IN int n_points, IN int m )
{
	...
	printf("*** GPU 1 - E:%d, N:%d, B:(%d,%d) M:global  GPU Time taken = %.3fms\n", 
...
void transform_points_SOA( INOUT POINTS_SOA p_SOA, IN int n_points, IN int m )
{
	...
	float elapsedTime;
	cudaEventElapsedTime(&elapsedTime, start, stop);
	printf("*** GPU 2 - E:%d, N:%d, B:(%d,%d) M:global  GPU Time taken = %.3fms\n", 
		ELEM_PER_POINT, N_ELEMS, BLOCK_WIDTH, BLOCK_HEIGHT, elapsedTime);
	cudaEventDestroy(start);
	cudaEventDestroy(stop);
...
void transform_points_AOS_with_constant( INOUT POINT_ELEMENT *p_AOS, IN int n_points, IN int m )
{
	...
	float elapsedTime;
	cudaEventElapsedTime(&elapsedTime, start, stop);
	printf("*** GPU 1 - E:%d, N:%d, B:(%d,%d) M:constnat  GPU Time taken = %.3fms\n", 
		ELEM_PER_POINT, N_ELEMS, BLOCK_WIDTH, BLOCK_HEIGHT, elapsedTime);
	cudaEventDestroy(start);
	cudaEventDestroy(stop);
}
...
void transform_points_SOA_with_constant( INOUT POINTS_SOA p_SOA, IN int n_points, IN int m )
{
	...
	float elapsedTime;
	cudaEventElapsedTime(&elapsedTime, start, stop);
	printf("*** GPU 2 - E:%d, N:%d, B:(%d,%d) M:constant  GPU Time taken = %.3fms\n", 
		ELEM_PER_POINT, N_ELEMS, BLOCK_WIDTH, BLOCK_HEIGHT, elapsedTime);
	cudaEventDestroy(start);
	cudaEventDestroy(stop);
}
...
int main(void){
	...
	float compute_time;
	int n_points, cutoff;
	POINT_ELEMENT *Points_AOS, *Const_Points_AOS;
	POINTS_SOA Points_SOA, Const_Points_SOA;
	...
	Const_Points_AOS = new POINT_ELEMENT[N_POINTS];
	memcpy(Const_Points_AOS, Points_AOS, N_ELEMS*sizeof(float));
	for( int i = 0; i < ELEM_PER_POINT; ++i )
	{
		Const_Points_SOA.elem[ i ] = new float[ N_POINTS ];
		memcpy(Const_Points_SOA.elem[i], Points_SOA.elem[ i ], N_POINTS*sizeof(float));
	}
	...
	printf("*** AOS.10.x = %e, SOA.10.x = %e\n", Const_Points_AOS[10].elem[0], Const_Points_SOA.elem[0][10]);
	printf("--- AOS.20.y = %e, SOA.20.y = %e\n", Const_Points_AOS[20].elem[1], Const_Points_SOA.elem[1][20]);
	...
	printf("*** GPU 1 AOS.10.x = %e, SOA.10.x = %e\n", Points_AOS[10].elem[0], Points_SOA.elem[0][10]);
	printf("--- AOS.20.y = %e, SOA.20.y = %e\n", Points_AOS[20].elem[1], Points_SOA.elem[1][20]);
	printf("\n");
